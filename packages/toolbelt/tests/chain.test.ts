...
it.skip('type check - long chain', () => {
  type X = Resolver<(result: 'I1') => 'ResResolver1', (error: 'E01') => 'ErrResolver'>
  const chainN0 = chain((x: 'start', resolve: X) => {
...
it('example usage', () =>
  new Promise((done) => {
    ...
    function addChar<T extends string, C extends string>(
      c: C,
    ): (
      input: T,
      resolver: {
        (result: `${C}:${T}`): void
        result: (result: `${C}:${T}`) => void
      },
    ) => void {
      return (input: T, resolver: Resolver<(result: `${C}:${T}`) => void>) =>
        resolver(`${c}:${input}` as `${C}:${T}`)
    }

    const a = addChar<'start', 'A'>('A')

    // adds three asynchronous functions to the chain
    const fooChain = chain(a, addChar<'A:start', 'B'>('B'), addChar<'B:A:start', 'C'>('C'))
...
it('basic', () =>
  new Promise((done) => {
    const chainy = chain(
      (x, resolve: Resolver<(output: string) => void, (error: Error) => void>) => {
...
it('basic3', () =>
  new Promise((done) => {
    ...
    checkType<(arg: 'start', resultCb: (result: boolean) => void) => AwaitedChainController<void>>(
      b.await,
    )
...
it('basic5', () =>
  new Promise((done) => {
    ...
    const x1 = doNotCall as unknown as (
      x: string,
      resolve: Resolver<(arg: string) => void, (error: string) => void>,
    ) => void
...
it('splicing chains2', () =>
  new Promise((done) => {
    ...
    const chainNumber = chain((x: string, resolve: Resolver<(out: number) => void>) => resolve(1))(
      (x: number, resolve: Resolver<(out: number) => void>) => resolve(x + 1),
    )
