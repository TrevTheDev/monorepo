...
> = [
  finalOptions: T & { transformObject: boolean; type: string },
  allKeys: PropertyKey[],
  // typeString: string,
  // transformObject: boolean,
]
...
>(
  ...
  const parsePropertySchemasFn = parsePropertySchemas(
    propertySchemas,
    finalOptions.missingProperty
      ? {
          missingProperty: finalOptions.missingProperty,
      ...
      ),
      {
        ...
        extends: {
          value(
            ...
            newOptions: Partial<ParseObjectOptions> = options,
          ) {
            return nextObject(
