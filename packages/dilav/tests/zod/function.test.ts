...
it('args method', () => {
  ...
  assertEqual<t1, (...args_1: never[]) => never>(true)

  const t2 = t1.args(v.string)
  type t2 = VInfer<typeof t2>
  assertEqual<t2, (arg: string) => never>(true)

  const t3 = t2.returns(v.boolean)
...
const args2 = vArray([
  vObject({
    f1: v.number,
    f2: v.string.nullable(),
    f3: vArray(v.boolean.optional()).optional(),
  }),
] as const)
const returns2 = vUnion([v.string, v.number])

const func2 = vFunction({ parameters: args2, returns: returns2 })
...
it('special function error codes', () => {
  const checker = vFunction({
    parameters: vArray([v.string] as const),
    returns: v.boolean,
...
it('function with async refinements', async () => {
  const func = vFunction({
    ...
    returns: v.promise(v.number.gt(10)),
...
it('non async function with async refinements should fail', async () => {
  const func = vFunction({
    args: [vStringInstance.min(10)],
    returns: v.promise(vNumberInstance.gt(10)),
...
it('allow extra parameters', () => {
  const maxLength5 = vFunction({
    parameters: vArray([vStringInstance, vArray(vAnyInstance).spread]),
    returns: vBooleanInstance,
...
it('params and returnType getters', () => {
  ...
  func.definition.parameters.definition.itemSchemas[0].parse('asdf')
  func.definition.returns.parse('asdf')
})
